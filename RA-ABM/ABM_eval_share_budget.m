function [results]=ABM_eval_share_budget(rain_field_true,radar_field,gauge_xy,...
...
N_event=length(rain_field_true);
	...
	[rain_field_esti,response,Incentive_rec]=ABM_share_budget(rain_field_true,radar_field,gauge_xy,...
																		budget,alpha_P,max_par,min_par,rate,...
																		X,Y,folder,loop_ID);
    id=1;                                      
    for i=1:N_event
        ...
        for j=1:T
        	...
        	results.cost(id,1)=sum(Incentive_rec{i,1}(:,j).*response{i,1}(:,j));
