function [results]=ABM_eval_historical_maxN(rain_field_true,radar_field,gauge_xy,...
											budget,alpha_P,max_par,min_par,rate,params,...
											X,Y,folder,N_cluster,loop_ID)
% function to run the ABM model (to maximize the total number of participants)
...
N_event=length(rain_field_true);
    ...
    [rain_field_esti,response,Incentive]=ABM_nonlinear_historical_curves_maxN(rain_field_true,radar_field,gauge_xy,...
    ...
    for i=1:N_event
        ...
        for j=1:T
            ...
            [results.ARE(id,1),results.RMSE(id,1)]=cal_rain_error(rain_field_true{i,1}(:,:,j),rain_field_esti{i,1}(:,:,j));
            id=id+1;
